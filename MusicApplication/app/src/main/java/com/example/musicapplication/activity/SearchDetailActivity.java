...
public class SearchDetailActivity extends AppCompatActivity {
    ...
    private List<Song> songsList = null;//歌曲列表
    ...
    private SongAdapter adapter_search;//listview适配器
    private ListView listView_search ;//listview对象
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        current_number = MusicService.getCurrent_number();//当前正在播放的歌曲
        current_status = MusicService.getCurrent_status();//获取当前情况
...
//                    DisplayActivity.setSongsList(songsList);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        //搜索框提示文字
        searchView.setQueryHint(this.getResources().getString(R.string.search_hint));//默认显示文字设置
...
//        searchView.setSubmitButtonEnabled(true);//提交按钮  显示
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String query) {
                if(!TextUtils.isEmpty(query)){//注意判空 如果不为空
                    search_list.clear();//搜索列表清空
                    adapter_search.notifyDataSetChanged();//更新listview适配器
                    listView_search.deferNotifyDataSetChanged();//更新listview
                    dealSearchAction(query);//搜索算法
                }
                return false;
            }
        });
...
/*搜索匹配算法*/
    private void dealSearchAction(String mes) {
        ...
        int num_songs = songsList.size();//获取本地歌曲列表的大小
        ...
        String current_song_name;//当前歌名
        char[] charArray_current_song_name;//当前歌名
        int cur_name_len;//当前歌名长度
...
//            Log.w("SearchDetailActivity", "目标长度"+mes_len+"    当前歌名长度"+cur_name_len);
            ...
            if (mes_len <= cur_name_len) {//如果目标歌名比当前歌名短或者长度相等
                ...
                if (percent_matching_degree == 1) {
                    ...
                    break;
                } else if (percent_matching_degree > 0.2) {//阈值为0.2  百分比匹配度大于阈值
                    search_list.add(songsList.get(i));
                }
            }
            //匹配度清零
