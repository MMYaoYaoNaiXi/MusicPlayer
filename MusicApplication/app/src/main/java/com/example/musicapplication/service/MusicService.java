...
public class MusicService extends Service {
    ...
    public static final int STATUS_STOPPED = 2;//停止播放
    public static final int STATUS_COMPLETED = 3;//播放完成
//    public static final int STATUS_PREVIOUS = 4;
    //progressBar相关
    public static final int PROGRESS_UPDATE = 4;//更新
    public static final int PROGRESS_DURATION = 5;//时长
    //播放模式已更新
    public static final int PLAY_MODE_UPDATE = 6;//已经更新
    //播放控制命令
    public static final int COMMAND_UNKNOWN = -1;
    public static final int COMMAND_PLAY = 0;//播放命令
    public static final int COMMAND_PAUSE = 1;//暂停命令
    //public static final int COMMAND_STOP = 2;
    public static final int COMMAND_RESUME = 3;//详情命令
    ...
    public void onCreate() {
        ...
        IntentFilter intentFilter = new IntentFilter(BROADCAST_MUSICSERVICE_CONTROL);// BROADCAST_MUSICSERVICE_CONTROL = "MusicService.ACTTION_CONTROL"
        registerReceiver(commandReceiver, intentFilter);//注册广播接收器
        //通知栏
    ...
    public void onDestroy() {
        ...
        unregisterReceiver(commandReceiver);//注销广播接收器
    ...
    private void sendServiceBroadcast(int content) {
        Intent intent = null;
        switch (content) {
            case PROGRESS_DURATION://时长
                intent = new Intent(BROADCAST_MUSICSERVICE_PROGRESS);
                intent.putExtra("content", content);
                intent.putExtra("duration", duration);//放入时长
                break;
            case PROGRESS_UPDATE://更新
                intent = new Intent(BROADCAST_MUSICSERVICE_PROGRESS);
                intent.putExtra("content", content);
    ...
    private void resume() {//暂停后的恢复播放
        player.start();
        current_status = MusicService.STATUS_PLAYING;//当前处于正在播放的状态
        sendServiceBroadcast(MusicService.STATUS_PLAYING);
        update_progress();//启动子线程更新进度条
        myNotification.notifyPlay(this);
    }
    ...
    private void moveNumberToPrevious() {
        if(current_number - 1 >= 0)//当前数歌曲数量大于1
            next_number = current_number - 1;
        play();//播放
    }
    /******内部类，接收耳机状态变化*/
